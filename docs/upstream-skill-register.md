...
| Future Entra portal skill | `entra-app-registration`, `entra-agent-id`, `Entra-POCAdvisor` | Build local Entra Admin Center portal skill as a router/safety layer. Route app registration and Agent ID work to official upstream skills where applicable. |
...
| Future Purview portal skill | `purview-dlm-mcp` | Build local Purview portal skill as a router/safety layer. Route DLM diagnostics to upstream when relevant. |
